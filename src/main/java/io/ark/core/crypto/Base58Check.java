...
/**
 * Converts between an array of bytes and a Base58Check string. Not
 * instantiable.
 */
public final class Base58Check {
	...
	// Adds the checksum and converts to Base58Check. Note that the caller needs to
	// prepend the version byte(s).
	...
	static byte[] addCheckHash(byte[] data) {
		try {
			byte[] hash = Arrays.copyOf(Crypto.getDoubleHash(data), 4);
	...
	// Converts the given Base58Check string to a byte array, verifies the checksum,
	// and removes the checksum to return the payload.
	...
	public static byte[] base58ToBytes(String s) {
		...
		byte[] rehash = Arrays.copyOf(Crypto.getDoubleHash(data), 4);
		if (!Arrays.equals(rehash, hash))
			throw new IllegalArgumentException("Checksum mismatch");
		return data;
	}

	// Converts the given Base58Check string to a byte array, without checking or
	// removing the trailing 4-byte checksum.
	...
	public static final String ALPHABET = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz"; // Everything
																										// except 0OIl
